...
volatile usart_rx_buf_t huart_l_rx_buf = {
    .last_msg_time = 0, .msg_size = MC_MAX_TX_LENGTH, .last_msg_loc = 0, .last_rx_time = 0, .rx_buf_size = MC_MAX_RX_LENGTH, .rx_buf = usart_l_rx_array};
...
volatile usart_rx_buf_t huart_r_rx_buf = {
    .last_msg_time = 0, .msg_size = MC_MAX_TX_LENGTH, .last_msg_loc = 0, .last_rx_time = 0, .rx_buf_size = MC_MAX_RX_LENGTH, .rx_buf = usart_r_rx_array};
...
void usartIdleIRQ(volatile usart_init_t *huart, volatile usart_rx_buf_t *rx_buf)
{
    ...
    if (new_loc == rx_buf->rx_buf_size)
        new_loc = 0; // should never happen
    else if (new_loc < rx_buf->last_rx_loc)
        new_loc += rx_buf->rx_buf_size;                   // wrap around
...
void latency_nav_CALLBACK(CanParsedData_t *msg_data_a)
{
    //last_lat_rx = time;
}
