...
void SFS_step(RT_MODEL *const rtM, ExtU *rtU, ExtY *rtY)
{
  ...
  quaternionBase_rotmat(IMUNED2VNED_tmp - 0.0 * IMUNED2VNED_tmp_0 * 0.0,
                        ...
                        IMUNED2VNED_tmp_0 - cos(ein_idx_1) * 0.0 * 0.0, cos(ein_idx_1) * 0.0 + 0.0 * sin(ein_idx_1), IMUNED2VNED);
  ...
  quaternionBase_rotmat(6.123233995736766E-17 * ein_idx_1 - IMUNED2VNED_tmp *
                                                                0.0,
                        6.123233995736766E-17 * IMUNED2VNED_tmp * 0.0 +
                            ein_idx_1,
                        6.123233995736766E-17 * IMUNED2VNED_tmp -
                            ein_idx_1 * 0.0,
                        6.123233995736766E-17 * ein_idx_1 * 0.0 + IMUNED2VNED_tmp, IMUNED2VNED);
  for (i = 0; i < 3; i++)
  {
    /* Outport: '<Root>/mag_VNED' */
    rtY->mag_VNED[i] = (IMUNED2VNED[i + 3] * rtU->mag[1] + IMUNED2VNED[i] *
                                                               rtU->mag[0]) +
                       IMUNED2VNED[i + 6] * rtU->mag[2];
