...
void imu_periodic(IMU_Handle_t *imu_h, ExtU *rtU)
{
    ...
    rtU->gyro[0] = CLAMP(gyro_in.x * GYRO_CALIBRATION, MIN_GYRO, MAX_GYRO);
    rtU->gyro[1] = CLAMP(gyro_in.y * GYRO_CALIBRATION, MIN_GYRO, MAX_GYRO);
    rtU->gyro[2] = CLAMP(gyro_in.z * GYRO_CALIBRATION, MIN_GYRO, MAX_GYRO);
    ...
    rtU->acc[0] = CLAMP(accel_in.x * ACC_CALIBRATION, MIN_ACC, MAX_ACC);
    rtU->acc[1] = CLAMP(accel_in.y * ACC_CALIBRATION, MIN_ACC, MAX_ACC);
    rtU->acc[2] = CLAMP(accel_in.z * ACC_CALIBRATION, MIN_ACC, MAX_ACC);
